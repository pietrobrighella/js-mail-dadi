    ...
    if (emailInserita != 0) {
        for (let i = 0; i < listaEmail.length; i++){
            ...
            if (emailInserita === listaEmail[i]){
                ...
                btnRegistra.addEventListener('click', function(){listaEmail.push(emailInserita);});
                btnRegistra.addEventListener('click', function(){newAlert.classList.add('d-none');})
                btnRegistra.addEventListener('click', function(){btnEmail.innerText = 'Gioca adesso!';})
            }
        } 
    } else {
        warningAlert.classList.remove('alert', 'alert-primary');
        warningAlert.innerHTML = `
        <div id="warningAlert" class="alert alert-danger">Inserisci un indirizzo email per giocare</div>
        `
    }
}
